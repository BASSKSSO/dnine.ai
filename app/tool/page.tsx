...
import { Upload, X, Download, RefreshCw, CircleAlert as AlertCircle, Image as ImageIcon, Loader as Loader2, Check, FileImage } from 'lucide-react';
    ...
    <div className="min-h-[calc(100vh-4rem)] px-4 sm:px-6 lg:px-8 py-8">
      <div className="mx-auto max-w-6xl">
        <div className="text-center mb-8">
          ...
          <p className="text-muted-foreground">Upload images and remove backgrounds with AI.</p>
          {user && <Badge variant="secondary" className="mt-3">{credits} credits remaining</Badge>}
        </div>

        {!user && (
          <div className="mb-6 flex items-center gap-3 justify-center rounded-lg bg-muted/50 p-4 text-sm">
        ...
        >
          ...
          <div className="flex flex-col items-center gap-4">
            <div className={`flex h-16 w-16 items-center justify-center rounded-2xl transition-colors ${isDragging ? 'bg-primary/20 text-primary' : 'bg-muted text-muted-foreground'}`}>
              <Upload className="h-8 w-8" />
            </div>
            <div>
              <p className="text-lg font-semibold">{isDragging ? 'Drop images here' : 'Drag & drop images here'}</p>
              <p className="text-sm text-muted-foreground mt-1">or click to browse. PNG, JPG, WEBP up to 10MB.</p>
            </div>
                ...
                <Card key={img.id} className="glass-card border-0 overflow-hidden">
                  <CardContent className="p-0">
                    <div className="relative aspect-square">
                      <img src={img.processed || img.original} alt={img.fileName} className={`w-full h-full object-cover ${img.processed ? 'checkerboard' : ''}`} />
                    ...
                    <div className="p-3 flex items-center justify-between">
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          {img.status === 'pending' && (
                            <Button size="sm" onClick={() => processImage(img)} disabled={!user || credits <= 0}>Process</Button>
                          )}
        ...
        {completedCount > 0 && (
          <div className="mt-8 text-center">
            <Button variant="outline" onClick={() => { setImages([]); setSelectedImage(null); }}>
              <RefreshCw className="h-4 w-4 mr-2" /> Process Another Image
            </Button>
          </div>
        )}
      </div>
